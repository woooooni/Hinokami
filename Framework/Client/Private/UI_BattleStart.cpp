...
CUI_BattleStart::CUI_BattleStart(const CUI_BattleStart & rhs)
	: CUI(rhs)
	, m_fAccDeletion(0.f)
	, m_fAlpha(1.f)
{
...
HRESULT CUI_BattleStart::Initialize_Prototype(const UI_INFO& tInfo)
{
	m_tInfo = tInfo;
...
HRESULT CUI_BattleStart::Ready_Components()
{
	...
	/* Com_Texture*/
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Texture_BattleStart"),
...
CUI_BattleStart * CUI_BattleStart::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const UI_INFO& tInfo)
{
	CUI_BattleStart*	pInstance = new CUI_BattleStart(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(tInfo)))
	{
		MSG_BOX("Failed to Created : CUI_BattleStart");
...
CGameObject * CUI_BattleStart::Clone(void* pArg)
{
	...
	{
		MSG_BOX("Failed to Cloned : CUI_BattleStart");
		Safe_Release(pInstance);
	}
