...
HRESULT CUI_Logo_BackGround::Initialize_Prototype(const UI_INFO& tInfo)
{
	m_tInfo = tInfo;
...
CUI_Logo_BackGround * CUI_Logo_BackGround::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext, const UI_INFO& tInfo)
{
	CUI_Logo_BackGround*	pInstance = new CUI_Logo_BackGround(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype(tInfo)))
	{
		MSG_BOX("Failed to Created : CUI_Logo_BackGround");
...
CGameObject * CUI_Logo_BackGround::Clone(void* pArg)
{
	...
	{
		MSG_BOX("Failed to Cloned : CUI_Logo_BackGround");
		Safe_Release(pInstance);
	}
