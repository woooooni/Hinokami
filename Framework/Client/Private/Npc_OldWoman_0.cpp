...
USING(Client)
CNpc_OldWoman_0::CNpc_OldWoman_0(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CNpc(pDevice, pContext, L"Npc_OldWoman_0")
{
	
}

CNpc_OldWoman_0::CNpc_OldWoman_0(const CNpc_OldWoman_0& rhs)
	: CNpc(rhs)	
...
HRESULT CNpc_OldWoman_0::Initialize_Prototype()
{
...
HRESULT CNpc_OldWoman_0::Initialize(void* pArg)
{
...
void CNpc_OldWoman_0::Tick(_float fTimeDelta)
{
	if (m_bReserveDead)
	{
...
void CNpc_OldWoman_0::LateTick(_float fTimeDelta)
{
...
HRESULT CNpc_OldWoman_0::Render()
{
	__super::Render();
...
HRESULT CNpc_OldWoman_0::Render_ShadowDepth()
{
...
void CNpc_OldWoman_0::Collision_Enter(const COLLISION_INFO& tInfo)
{

}

void CNpc_OldWoman_0::Collision_Continue(const COLLISION_INFO& tInfo)
{
...
void CNpc_OldWoman_0::Collision_Exit(const COLLISION_INFO& tInfo)
{
...
void CNpc_OldWoman_0::On_Damaged(CGameObject* pAttacker, _uint eDamageType, _float fDamage)
{
	__super::On_Damaged(pAttacker, eDamageType, fDamage);
}

HRESULT CNpc_OldWoman_0::Ready_Components()
{
	/* For.Com_Transform */
	...
	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Model_Npc_OldWoman_0"), TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CNpc_OldWoman_0::Ready_States()
{
	list<wstring> strAnimationName;
	strAnimationName.push_back(L"SK_N0037_V00_C00.ao|A_N0037_STA001_Nut_02LP");
...
HRESULT CNpc_OldWoman_0::Ready_Colliders()
{
...
CNpc_OldWoman_0* CNpc_OldWoman_0::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CNpc_OldWoman_0* pInstance = new CNpc_OldWoman_0(pDevice, pContext);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Create Failed : CNpc_OldWoman_0");
...
CGameObject* CNpc_OldWoman_0::Clone(void* pArg)
{
	CNpc_OldWoman_0* pInstance = new CNpc_OldWoman_0(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed to Cloned : CNpc_OldWoman_0");
		Safe_Release(pInstance);
	}
